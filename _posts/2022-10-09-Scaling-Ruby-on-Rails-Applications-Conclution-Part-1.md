---
layout: posts
title: Scaling Ruby on Rails Applications Conclution - Part 1
icon: fa-comment-alt
...
The proposed improvements were all about caching and if it was worth increasing com- plexity to increase performance. To do so, we divided our tests into two groups; first, we analysis simple caching, just leaving Rails to cache what it "thought" was important and caching translations. We jumped into the caching page and partials to identify if perfor- mance increases and if it is worth the performance difference due to managing cache state on small pieces of code updates. It is also important to keep in mind that our research has shown us that cache is one of the main causes of bugs in enterprise level applications.
